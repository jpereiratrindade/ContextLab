...
[[nodiscard]] constexpr uint32_t choose(uint32_t e, uint32_t f, uint32_t g) noexcept {
    return (e & f) ^ (~e & g);
}

[[nodiscard]] constexpr uint32_t majority(uint32_t a, uint32_t b, uint32_t c) noexcept {
    return (a & b) ^ (a & c) ^ (b & c);
}
...
[[nodiscard]] constexpr uint32_t theta0(uint32_t x) noexcept {
    return rotr(x, 7) ^ rotr(x, 18) ^ (x >> 3);
}

[[nodiscard]] constexpr uint32_t theta1(uint32_t x) noexcept {
...
std::string Sha256::finalize() {
    ...
    for (size_t idx = 0; idx < 8; ++idx) {
        ss << std::setw(8) << state_[idx];
    }
    return ss.str();
}

std::string Sha256::hashString(std::string_view data) {
    Sha256 hasher;
    hasher.update(data);
    return hasher.finalize();
}

std::string Sha256::hashBytes(std::span<const uint8_t> data) {
    Sha256 hasher;
    hasher.update(data);
    return hasher.finalize();
}

std::string Sha256::hashFile(const std::filesystem::path& path) {
    ...
    while (file.read(buffer.data(), buffer.size()) || file.gcount() > 0) {
        hasher.update(std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(buffer.data()), static_cast<size_t>(file.gcount())));
    }
    return hasher.finalize();
}
